...
export const getSubModuleColours = (subModuleNames) =>  subModuleNames.reduce((acc, entry) => {
  acc.push({
    name: entry,
...
export const getSimulation = (d3,graphDataType, linkForceStrength,radiusCollideMultiplier,hierarchyData,width, height) => {
  const xWeight = width > height ? 0.7 : 1;
  const yWeight = width > height ? 1 : 0.7;

  const getSubModulePositions = () => {
    const submoduleLeaves = hierarchyData.subModuleNodes.map((m) =>  ({name: m.id || m.data?.id,value: d3.sum(m.leaves(), (s) => s.data.linkCount)}));
